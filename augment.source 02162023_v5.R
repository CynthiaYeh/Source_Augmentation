...
datSource <- datSource %>%
  ...
  dplyr::select(!all_of(internalCols) & !date.transform)
...
# Marked as pending if include == "yes"
datSource$database <- ifelse(datSource$source.number %in% sDB.include.yes, "pending", datSource$database)

# Marked as no
datSource$database <- ifelse(datSource$database == "", "no", datSource$database)
...
# Find if 
unique(datSource$database[!datSource$database %in% yesNo]) #should be empty
